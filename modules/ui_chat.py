...
from modules import chat, shared, ui, utils, i18n
...
def create_ui():
    ...
    with gr.Tab(i18n.t('Chat'), elem_id='chat-tab'):
        with gr.Row(elem_id='past-chats-row', elem_classes=['pretty_scrollbar']):
            with gr.Column():
                with gr.Row(elem_id='past-chats-buttons'):
                    ...
                    shared.gradio['Start new chat'] = gr.Button('New chat', elem_classes=['refresh-button', 'refresh-button-medium', 'focus-on-chat-input'], elem_id='Start-new-chat')
            ...
            with gr.Column(elem_id='chat-col'):
                ...
                with gr.Row(elem_id="chat-input-row"):
                    ...
                    with gr.Column(scale=10, elem_id='chat-input-container'):
                        shared.gradio['textbox'] = gr.MultimodalTextbox(label='', placeholder=i18n.t('Send a message'), file_types=['text', '.pdf', 'image'], file_count="multiple", elem_id='chat-input', elem_classes=['add_scrollbar'])
                        shared.gradio['typing-dots'] = gr.HTML(value='<div class="typing"><span></span><span class="dot1"></span><span class="dot2"></span></div>', label='typing', elem_id='typing-container')

                    with gr.Column(scale=1, elem_id='generate-stop-container'):
                        with gr.Row():
                            shared.gradio['Stop'] = gr.Button(i18n.t('Stop'), elem_id='stop', visible=False)
                            shared.gradio['Generate'] = gr.Button(i18n.t('Generate'), elem_id='Generate', variant='primary')

        # Hover menu buttons
        with gr.Column(elem_id='chat-buttons'):
            shared.gradio['Regenerate'] = gr.Button(i18n.t('Regenerate') + ' (Ctrl + Enter)', elem_id='Regenerate')
            shared.gradio['Continue'] = gr.Button(i18n.t('Continue') + ' (Alt + Enter)', elem_id='Continue')
            shared.gradio['Remove last'] = gr.Button(i18n.t('Remove last') + ' (Ctrl + Shift + Backspace)', elem_id='Remove-last')
            shared.gradio['Impersonate'] = gr.Button(i18n.t('Impersonate') + ' (Ctrl + Shift + M)', elem_id='Impersonate')
            shared.gradio['Send dummy message'] = gr.Button(i18n.t('Send dummy message'))
            shared.gradio['Send dummy reply'] = gr.Button(i18n.t('Send dummy reply'))
            shared.gradio['send-chat-to-notebook'] = gr.Button(i18n.t('Send to Notebook'))
            shared.gradio['show_controls'] = gr.Checkbox(value=shared.settings['show_controls'], label=i18n.t('Show controls') + ' (Ctrl+S)', elem_id='show-controls')

        with gr.Row(elem_id='chat-controls', elem_classes=['pretty_scrollbar']):
            with gr.Column():
                with gr.Row():
                    shared.gradio['start_with'] = gr.Textbox(label=i18n.t('Start reply with'), placeholder='Sure thing!', value=shared.settings['start_with'], elem_classes=['add_scrollbar'])

                gr.HTML("<div class='sidebar-vertical-separator'></div>")

                shared.gradio['reasoning_effort'] = gr.Dropdown(value=shared.settings['reasoning_effort'], choices=['low', 'medium', 'high'], label=i18n.t('Reasoning effort'), info='Used by GPT-OSS.')
                shared.gradio['enable_thinking'] = gr.Checkbox(value=shared.settings['enable_thinking'], label=i18n.t('Enable thinking'), info='Used by Seed-OSS and pre-2507 Qwen3.')

                gr.HTML("<div class='sidebar-vertical-separator'></div>")

                shared.gradio['enable_web_search'] = gr.Checkbox(value=shared.settings.get('enable_web_search', False), label=i18n.t('Activate web search'), elem_id='web-search')
                with gr.Row(visible=shared.settings.get('enable_web_search', False)) as shared.gradio['web_search_row']:
                    shared.gradio['web_search_pages'] = gr.Number(value=shared.settings.get('web_search_pages', 3), precision=0, label=i18n.t('Number of pages to download'), minimum=1, maximum=10)

                gr.HTML("<div class='sidebar-vertical-separator'></div>")

                with gr.Row():
                    shared.gradio['mode'] = gr.Radio(choices=['instruct', 'chat-instruct', 'chat'], value=None, label=i18n.t('Mode'), info='In instruct and chat-instruct modes, the template under Parameters > Instruction template is used.', elem_id='chat-mode')

                with gr.Row():
                    shared.gradio['chat_style'] = gr.Dropdown(choices=utils.get_available_chat_styles(), label=i18n.t('Chat style'), value=shared.settings['chat_style'], visible=shared.settings['mode'] != 'instruct')
...
def create_character_settings_ui():
    mu = shared.args.multi_user
    with gr.Tab(i18n.t('Character'), elem_id="character-tab"):
        with gr.Row():
            with gr.Column(scale=8):
                with gr.Tab(i18n.t("Character")):
                    with gr.Row():
                        shared.gradio['character_menu'] = gr.Dropdown(value=shared.settings['character'], choices=utils.get_available_characters(), label=i18n.t('Character'), elem_id='character-menu', info='Used in chat and chat-instruct modes.', elem_classes='slim-dropdown')
                    ...
                    shared.gradio['name2'] = gr.Textbox(value=shared.settings['name2'], lines=1, label=i18n.t('Character\'s name'))
                    shared.gradio['context'] = gr.Textbox(value=shared.settings['context'], lines=10, label=i18n.t('Context'), elem_classes=['add_scrollbar'], elem_id="character-context")
                    shared.gradio['greeting'] = gr.Textbox(value=shared.settings['greeting'], lines=5, label=i18n.t('Greeting'), elem_classes=['add_scrollbar'], elem_id="character-greeting")

                with gr.Tab("User"):
                    shared.gradio['name1'] = gr.Textbox(value=shared.settings['name1'], lines=1, label=i18n.t('Name'))
                    shared.gradio['user_bio'] = gr.Textbox(value=shared.settings['user_bio'], lines=10, label=i18n.t('Description'), info='Here you can optionally write a description of yourself.', placeholder='{{user}}\'s personality: ...', elem_classes=['add_scrollbar'], elem_id="user-description")

                with gr.Tab('Chat history'):
                    with gr.Row():
                        with gr.Column():
                            shared.gradio['save_chat_history'] = gr.Button(value=i18n.t('Save history'))

                        with gr.Column():
                            shared.gradio['load_chat_history'] = gr.File(type='binary', file_types=['.json', '.txt'], label=i18n.t('Upload History JSON'))

                with gr.Tab('Upload character'):
                    with gr.Tab('YAML or JSON'):
                        with gr.Row():
                            shared.gradio['upload_json'] = gr.File(type='binary', file_types=['.json', '.yaml'], label=i18n.t('JSON or YAML File'), interactive=not mu)
                            shared.gradio['upload_img_bot'] = gr.Image(type='filepath', label=i18n.t('Profile Picture (optional)'), interactive=not mu)

                        shared.gradio['Submit character'] = gr.Button(value=i18n.t('Submit'), interactive=False)

                    with gr.Tab('TavernAI PNG'):
                        with gr.Row():
                            with gr.Column():
                                shared.gradio['upload_img_tavern'] = gr.Image(type='filepath', label=i18n.t('TavernAI PNG File'), elem_id='upload_img_tavern', interactive=not mu)
                                shared.gradio['tavern_json'] = gr.State()
                            with gr.Column():
                                shared.gradio['tavern_name'] = gr.Textbox(value='', lines=1, label=i18n.t('Name'), interactive=False)
                                shared.gradio['tavern_desc'] = gr.Textbox(value='', lines=10, label=i18n.t('Description'), interactive=False, elem_classes=['add_scrollbar'])

                        shared.gradio['Submit tavern character'] = gr.Button(value=i18n.t('Submit'), interactive=False)

            with gr.Column(scale=1):
                shared.gradio['character_picture'] = gr.Image(label=i18n.t('Character picture'), type='filepath', interactive=not mu)
                shared.gradio['your_picture'] = gr.Image(label=i18n.t('Your picture'), type='filepath', value=Image.open(Path('user_data/cache/pfp_me.png')) if Path('user_data/cache/pfp_me.png').exists() else None, interactive=not mu)
...
def create_chat_settings_ui():
    ...
    with gr.Tab('Instruction template'):
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    shared.gradio['instruction_template'] = gr.Dropdown(choices=utils.get_available_instruction_templates(), label=i18n.t('Instruction template'), info="After selecting the template, click on \"Load\" to load and apply it.", value='None', elem_classes='slim-dropdown')
                    ...
                    shared.gradio['load_template'] = gr.Button(i18n.t("Load"), elem_classes='refresh-button')
        ...
        with gr.Row():
            with gr.Column():
                shared.gradio['instruction_template_str'] = gr.Textbox(value=shared.settings['instruction_template_str'], label=i18n.t('Instruction template'), lines=24, info='This gets autodetected; you usually don\'t need to change it. Used in instruct and chat-instruct modes.', elem_classes=['add_scrollbar', 'monospace'], elem_id='instruction-template-str')
                with gr.Row():
                    shared.gradio['send_instruction_to_notebook'] = gr.Button('Send to notebook', elem_classes=['small-button'])

            with gr.Column():
                shared.gradio['chat_template_str'] = gr.Textbox(value=shared.settings['chat_template_str'], label=i18n.t('Chat template'), lines=22, elem_classes=['add_scrollbar', 'monospace'], info='Defines how the chat prompt in chat/chat-instruct modes is generated.', elem_id='chat-template-str')
